...
def generate_metadata_from_content():
    """
    Auto-generate video title and description based on previous steps
    
    Returns:
        tuple: (title, description)
    """
    # Default values
    default_title = "My AI-Generated Short Video"
    default_description = "This video was created with AI Money Printer Shorts app.\n\n#AIVideo #Shorts #ContentCreation"
    
    title = default_title
    description = default_description
    
    try:
        # Check if we have access to blueprint data (topic title)
        if "blueprint" in st.session_state and st.session_state.blueprint.get("topic_title"):
            title = st.session_state.blueprint["topic_title"]
            
        # Check if we have access to script data
        script_content = ""
        if "script_segmentation" in st.session_state and st.session_state.script_segmentation.get("full_script"):
            script_content = st.session_state.script_segmentation["full_script"]
        
        # If we have both title and script, generate a better description
        if title != default_title and script_content:
            # Extract first few sentences for description (max 200 chars)
            short_summary = script_content[:200].strip()
            if len(script_content) > 200:
                short_summary += "..."
                
            # Create description with summary and hashtags
            keywords = extract_keywords_from_script(script_content)
            hashtags = generate_hashtags(keywords, title)
            
            description = f"{short_summary}\n\n{hashtags}"
    except Exception as e:
        st.warning(f"Could not auto-generate metadata: {str(e)}")
    
    return title, description

def extract_keywords_from_script(script):
    """Extract relevant keywords from the script content"""
    # Simple extraction by splitting and filtering
    # A more advanced version could use NLP for keyword extraction
    words = script.lower().split()
    # Filter common words
    common_words = {"the", "and", "a", "to", "of", "in", "is", "that", "it", "for", "you", "with", 
                    "on", "this", "be", "are", "as", "at", "have", "by", "or", "not", "from"}
    keywords = [word for word in words if len(word) > 3 and word not in common_words]
    
    # Count frequency and return top 10 unique keywords
    keyword_count = {}
    ...
    # Sort by frequency and return up to 10 unique keywords
    sorted_keywords = sorted(keyword_count.items(), key=lambda x: x[1], reverse=True)
    return [word for word, _ in sorted_keywords[:10]]

def generate_hashtags(keywords, title):
    """Generate hashtags from keywords and title"""
    # Add hashtags from title words
    title_words = title.lower().split()
    
    # Create camel case hashtags (e.g., #ArtificialIntelligence)
    hashtags = []
    ...
    standard_hashtags = ["#AIVideo", "#Shorts", "#ContentCreation"]
    for tag in standard_hashtags:
        if tag not in hashtags:
            hashtags.append(tag)
    
    # Return the top 7 hashtags as a string
...
def main():
    ...
    if st.session_state.social_media_upload["source_video"]:
        ...
        # Common metadata - auto-generate from previous steps
        st.subheader("Video Metadata")
        
        # Auto-generate title and description
        auto_generated_title, auto_generated_description = generate_metadata_from_content()
        
        # Option to use auto-generated metadata
        use_auto_metadata = st.checkbox(
            "Use auto-generated metadata",
            value=True,
            help="Automatically generate title and description from your script and topic"
        )
        
        if use_auto_metadata:
            title_value = auto_generated_title
            description_value = auto_generated_description
            st.success("✅ Auto-generated metadata from your script and topic title")
        else:
            title_value = "My AI-Generated Short Video"
            description_value = "This video was created with AI Money Printer Shorts app.\n\n#AIVideo #Shorts #ContentCreation"
        ...
        if description:
            ...
            for word in description.split():
                if word.startswith("#"):
                    tag = word[1:]  # Remove the # character
        ...
        tags_input = st.text_input(
            ...
            value=", ".join(tags) if tags else "AIVideo, Shorts, ContentCreation",
