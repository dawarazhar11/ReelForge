...
echo -e "${BLUE}┌─────────────────────────────────────────────┐${NC}"
echo -e "${BLUE}│  ${GREEN}AI Money Printer - Fix Repeated Segments${BLUE}  │${NC}"
echo -e "${BLUE}└─────────────────────────────────────────────┘${NC}"
echo ""

# Check if we're in the right directory
if [ ! -f "fix_assembly_sequence.py" ]; then
    echo -e "${RED}Error: fix_assembly_sequence.py not found in current directory.${NC}"
...
chmod +x fix_assembly_sequence.py

# Run the fix script
echo -e "${YELLOW}Running assembly sequence fix script...${NC}"
python3 fix_assembly_sequence.py

# Check if the script ran successfully
if [ $? -eq 0 ]; then
    echo -e "${GREEN}Fix complete!${NC}"
    echo -e "${YELLOW}Now you can run the Video Assembly page to create a new video without repeated segments.${NC}"
    ...
    echo -e "${RED}Error: Failed to fix assembly sequence.${NC}"
